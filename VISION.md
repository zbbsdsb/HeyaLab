...
### 1. AI Proposes, Human Decides

The AI does the heavy lifting — reasoning, generating, verifying, remembering. Humans make the calls that matter — architecture choices, quality standards, direction changes.
...
Existing AI tools have fundamental limitations:

| Tool | Limitation |
|------|-----------|
| ChatGPT / Claude | Session-based, no project memory, text-only output |
| Cursor / Copilot | File-level context, code-only, no orchestration |
...
These principles will never be compromised:

1. **Humans always have the final say** on high-stakes decisions
